			...
			if (bic->sizeA - i >= minRow)
			{
				...
				else if (bic->sizeA > minRow && qnmv >= minRow) // otherwise, bic r can not generate descendants with at least minRow rows
				{
					...
					while (p1 <= qnmv - minRow)
					{
						...
						{
							pskipJ = false;
							bool iscan = RCHVP_IsCanonical(D, j, p1, p2, bic, fcol);
							if (iscan)
							{
								naux1 = false;
...
bool RCHVP_IsCanonical(const dataset_t &D, const col_t &y, const row_t &p1, const row_t &p2, const pbic_t &bic, col_t &fcol)
{
	data_t difpivo;
	row_t i;
	for (col_t j = 0; j < y; ++j)
