...
void RCVC_computeRM(const data_t &epsilon, const pbic_t &bic, const pbic_t &child, const row_t &p1, const row_t &p2, const row_t qnmv)
{
	...
	if (p1 > 0)
	{
		...
		row_t posPivo = p1 + g_smallerMinsup - 1, p = p1 - 1;
	...
	if (p2 < qnmv - 1)
	{
		row_t posPivo = p2 - g_smallerMinsup + 1, p = p2 + 1;
...
void RInCloseCVCve(const dataset_t &D, const row_t &n, const col_t &m, const col_t &minCol, const data_t *epsilons, const pbic_t &bic)
{
	...
	for (col_t j = bic->col; j < m; ++j)
	{
		...
		if (!bic->B[j] && !bic->PN[j])
		{
			...
			{//bic->biggerSup==bic->sizeA means conf=100%. So, I am cutting the branch when I find a bic with conf=100%.
				...
				while (p1 <= qnmv - g_smallerMinsup && p2 < qnmv - 1)
				{
					if (p2 < p1) p2 = p1;
					while (p2 < qnmv - 1 && g_RWp[p2 + 1].first - g_RWp[p1].first <= epsilons[j]) ++p2;
					biggerSup = 0;
					for (unsigned short i = 0; i < g_maxLabel; ++i) support[i] = 0; // initialize vector
					row_t sRW = 0;
					for (row_t i2 = p1; i2 <= p2; ++i2)
					{
						label = g_classes[g_RWp[i2].second];
						support[label]++;
	...
	while (!children.empty())
	{
		...
		RInCloseCVCve(D, n, m, minCol, epsilons, child);
		children.pop();
	}
	delete[] bic->B;
	delete[] bic->PN;
